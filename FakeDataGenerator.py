...
if scale_variation:
    lower_bound_scale = float(
        input("Enter the lower bound for scale variation (default 0.5): ") or "0.5"
    )
    upper_bound_scale = float(
        input("Enter the upper bound for scale variation (default 1.5): ") or "1.5"
    )

color_variation = input("Vary the color of the cursors? Y/N: ").upper() == "Y"
if color_variation:
    lower_bound_color = float(
        input("Enter the lower bound for color variation factor (default 0.9): ")
        or "0.9"
    )
    upper_bound_color = float(
        input("Enter the upper bound for color variation factor (default 1.1): ")
        or "1.1"
    )
...
prepare_directories(
    "", [train_images_dir, val_images_dir, train_labels_dir, val_labels_dir]
)
...
for idx, file in enumerate(cursor_files):
    ...
    cursor_templates.append(
        (make_background_transparent(cv2.imread(file, cv2.IMREAD_UNCHANGED)), idx)
    )
...
while cap.isOpened():
    ...
    if scale_variation:
        ...
        cursor = cv2.resize(
            cursor, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_AREA
        )
    ...
    for c in range(0, 3):
        frame[
            y_offset : y_offset + cursor.shape[0],
            x_offset : x_offset + cursor.shape[1],
            c,
        ] = (
...
if frame_paths:
    train_paths, val_paths = train_test_split(
        frame_paths, test_size=0.2, random_state=42
    )

    for path in val_paths:
        shutil.move(path, os.path.join(val_images_dir, os.path.basename(path)))
        label_path = path.replace(".png", ".txt").replace(
            train_images_dir, train_labels_dir
        )
        shutil.move(
            label_path, os.path.join(val_labels_dir, os.path.basename(label_path))
        )
